...
contract HelperConfigTest is Test, codeConstants {
    ...
    function testGetSepoliaConfig() public {
        ...
        assertEq(
            ...
            "Incorrect LINK token address for Sepolia"
        );
        assertEq(
        ...
        assertEq(
            config.account,
            0x3230AFf184EF0C815C0163d8bbbD53CCaB420A9d,
            "Incorrect account address for Sepolia"
        );
    ...
    function testGetOrCreateAnvilConfig() public {
        ...
        assertTrue(
            config.link != address(0),
            "LINK token address should not be zero"
        );
        assertEq(
            config.subscriptionId,
            0,
            "Subscription ID should be zero for local Anvil"
        );
        assertEq(
            config.account,
            0x1804c8AB1F12E6bbf3894d4083f33e07309d1f38,
            "Incorrect account address for local Anvil"
        );
    ...
    function testMockContractsDeployed() public {
        ...
        assertEq(
            balance,
            0,
            "VRF Coordinator subscription balance should be zero"
        );
        ...
        assertEq(
            ...
            0,
            "LINK token balance should be zero"
        );
